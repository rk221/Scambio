        ...
        describe 'ユーザでログインしている場合' do
            ...
            describe 'アイテムトレード一覧表示' do
                ...
                context '期限、有効フラグ共に有効なアイテムトレードを登録する' do
                    let(:item_trade){FactoryBot.build(:item_trade, user_id: login_user.id, game_id: game.id, buy_item_id: buy_item.id, sale_item_id: sale_item.id, enable_flag: true, numeric_of_trade_deadline: 1, user_game_rank_id: user_game_rank.id)}
                    before do 
                ...
                context '有効フラグが無効なアイテムトレードを登録する' do
                    let(:item_trade){FactoryBot.build(:item_trade, user_id: login_user.id, game_id: game.id, buy_item_id: buy_item.id, sale_item_id: sale_item.id, enable_flag: false, numeric_of_trade_deadline: 1, user_game_rank_id: user_game_rank.id)}
                    before do 
                        @item_trade = item_trade
                        @item_trade.save
                        @item_trade_queue = FactoryBot.create(:item_trade_queue, user_id: nil, enable_flag: true, item_trade_id: @item_trade.id)
